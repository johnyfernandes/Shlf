...
private func updateAppData(
    ...
    do {
        ...
        // Create auto-generated reading session
        let session = ReadingSession(
            startPage: startPage,
            endPage: newPage,
            durationMinutes: max(1, pagesRead * 2), // Estimated: 2 min/page
            xpEarned: pagesRead * 10, // Base XP only for quick updates
            isAutoGenerated: true,
            book: book
        )
        context.insert(session)

        // Get user profile and award XP
        let profileDescriptor = FetchDescriptor<UserProfile>()
        let profiles = try context.fetch(profileDescriptor)

        if let profile = profiles.first {
            profile.totalXP += session.xpEarned

            // Update goals
            let tracker = GoalTracker(modelContext: context)
            tracker.updateGoals(for: profile)

            logger.info("✅ Awarded \(session.xpEarned) XP to profile")
        }
        ...
        // Sync to Watch
        syncToWatch(bookUUID: book.id, pageDelta: pagesRead, profile: profiles.first)
